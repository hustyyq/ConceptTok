...
from data import SimpleImageDataset, PretoeknizedDataSetJSONL, PretokenizedWebDataset, ParquetImageDataset
import torch
from torch.utils.data import DataLoader
from omegaconf import OmegaConf
from torch.optim import AdamW
from utils.lr_schedulers import get_scheduler
from modeling.modules import EMAModel, ReconstructionLoss_Single_Stage, ConceptReconstructionLoss_Single_Stage, PConceptReconstructionLoss_Single_Stage, MLMLoss
from modeling.titok import TiTok, PretrainedTokenizer
from modeling.tatitok import TATiTok, TextEncTiTok, TexTTiTok
...
from utils.viz_utils import make_viz_from_samples, make_viz_from_samples_t2i_generation
...
def create_model_and_loss_module(config, logger, accelerator,
    ...
    if model_type == "titok":
        model_cls = TiTok
        # loss_cls = ReconstructionLoss_Single_Stage if config.model.vq_model.finetune_decoder else ReconstructionLoss_Stage1
        loss_cls = ReconstructionLoss_Single_Stage  
    elif model_type == "titok_lc":
        model_cls = TiTok
        loss_cls = ConceptReconstructionLoss_Single_Stage
    elif model_type == "tatitok":
        model_cls = TATiTok
        loss_cls = ReconstructionLoss_Single_Stage
    elif model_type == "tatitok_lc":
        model_cls = TATiTok
        loss_cls = ConceptReconstructionLoss_Single_Stage
    elif model_type == "textenc_titok":
        model_cls = TextEncTiTok
        loss_cls = ReconstructionLoss_Single_Stage
    elif model_type == "textenc_titok_lc":
        model_cls = TextEncTiTok
        loss_cls = ConceptReconstructionLoss_Single_Stage
        model_type =  "textenc_titok"
    elif model_type == "textenc_titok_pc":
        model_cls = TextEncTiTok
        loss_cls = PConceptReconstructionLoss_Single_Stage
    elif model_type == "text_titok":
        model_cls = TexTTiTok
        loss_cls = ReconstructionLoss_Single_Stage
    ...
    if accelerator.is_main_process:
        if model_type in ["titok", "titok_lc"]:
        ...
        elif model_type in ["tatitok", "tatitok_lc", "textenc_titok", "text_titok", "textenc_titok_pc"]:
...
def create_dataloader(config, logger, accelerator):
    ...
    if dataset_config.get("dataset_with_text_label", False) is True and not dataset_config.get("train_shards_path_or_url").endswith(".parquet"):
    ...
    elif dataset_config.get("dataset_with_text_label", False) is True and dataset_config.get("train_shards_path_or_url", "").endswith(".parquet"):
        dataset = ParquetImageDataset(
            train_parquet_path=dataset_config.train_shards_path_or_url,
            eval_parquet_path=dataset_config.eval_shards_path_or_url,
            num_train_examples=config.experiment.max_train_examples,
            per_gpu_batch_size=config.training.per_gpu_batch_size,
            global_batch_size=total_batch_size_without_accum,
            num_workers_per_gpu=dataset_config.num_workers_per_gpu,
...
def train_one_epoch(config, logger, accelerator,
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if model_type in ["tatitok", "tatitok_lc", "textenc_titok", "text_titok", "textenc_titok_pc"]:
            if "text" in batch:
                text = batch["text"]
            else:
        ...
        if accelerator.sync_gradients:
            ...
            if (global_step + 1) % config.experiment.generate_every == 0 and accelerator.is_main_process:
                ...
                reconstruct_images(
                    ...
                    text_guidance=text_guidance[:config.training.num_generated_images] if model_type in ["tatitok", "tatitok_lc", "textenc_titok", "text_titok", "textenc_titok_pc"] else None,
            ...
            # if eval_dataloader is not None and ((global_step + 1) % config.experiment.eval_every == 0 or global_step == 0):
...
def train_one_epoch_t2i_generator(
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            if "textenc" in config.experiment.project:
                text_guidance = clip_tokenizer(captions).to(accelerator.device)
                cast_dtype = clip_encoder.transformer.get_cast_dtype()
                text_guidance = clip_encoder.token_embedding(text_guidance).to(cast_dtype)  # [batch_size, n_ctx, d_model]
                text_guidance = text_guidance + clip_encoder.positional_embedding.to(cast_dtype)
                text_guidance = text_guidance.permute(1, 0, 2)  # NLD -> LND
                ...
                text_guidance = text_guidance.permute(1, 0, 2)  # LND -> NLD
                text_guidance = clip_encoder.ln_final(text_guidance)  # [batch_size, n_ctx, transformer.width]
                input_tokens = tokenizer.encode(images, text_guidance)[1]["min_encoding_indices"].squeeze(1)
            elif "gigatok" in config.experiment.project:
                # input_tokens = tokenizer.encode(images)[1]["min_encoding_indices"].squeeze(1)
                latent, _, [_, _, input_tokens] = tokenizer.encode(images)
        ...
        if model_type != "maskgen_vq":
            raise NotImplementedError(f"Unsupported t2i generator model_type {model_type}")
...
def t2i_generate_images(model, tokenizer, captions, aes_scores, clip_tokenizer, clip_encoder, accelerator, 
                    global_step, output_dir, logger, config=None, model_type="maskgen_vq"):
    model.eval()
    tokenizer.eval()
    local_model = accelerator.unwrap_model(model)
    ...
    tokens = local_model.generate(
        ...
        num_sample_steps=config.model.maskgen.get("num_iter", 16),
    ...
    if "gigatok" in config.experiment.project:
        samples = tokenizer.decode_code(tokens.reshape(-1), (tokens.shape[0], 8, 1, 256))
        generated_image = torch.clamp(127.5 * samples + 128.0, 0, 255)/255.0
    elif "tatitok" in config.experiment.project:
        generated_image = tokenizer.decode_tokens(tokens, text_guidance)
    else:
        generated_image = tokenizer.decode_tokens(tokens)
