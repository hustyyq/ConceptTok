...
from modeling.modules.concept_loss import LCL, PCL
...
class TiTok(BaseModel, PyTorchModelHubMixin, tags=["arxiv:2406.07550", "image-tokenization"], repo_url="https://github.com/bytedance/1d-tokenizer", license="apache-2.0"):
    def __init__(self, config):
        ...
        if self.quantize_mode == "vq":
            self.quantize = VectorQuantizer(
                ...
                clustering_vq=config.model.vq_model.get("clustering_vq", False),
    ...
    def encode(self, x):
        if self.finetune_decoder:
            ...
            if self.encoder.is_latent_concept_loss:
                z, concept_proj = self.encoder(pixel_values=x, latent_tokens=self.latent_tokens)
            else:
                z = self.encoder(pixel_values=x, latent_tokens=self.latent_tokens)
            if self.quantize_mode == "vq":
                z_quantized, result_dict = self.quantize(z)
