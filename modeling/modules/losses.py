...
class ReconstructionLoss_Stage2(torch.nn.Module):
    def __init__(
        ...
        self.quantizer_weight = loss_config.quantizer_weight
        self.entropy_annealing_steps = loss_config.get("entropy_annealing_steps", 4000)
...
class ConceptReconstructionLoss_Single_Stage(ReconstructionLoss_Stage2):
    def __init__(
        self,
        config
    ):
        ...
        loss_config = config.losses
        self.quantize_mode = config.model.vq_model.get("quantize_mode", "vq")
        self.latent_concept_weight = loss_config.latent_concept_weight
    ...
    def _forward_generator(self,
        ...
        if self.quantize_mode == "vq":
            # Compute quantizer loss.
            quantizer_loss = extra_result_dict["quantizer_loss"]
            latent_concept_loss =  extra_result_dict["latent_concept_loss"]
            if self.entropy_annealing_factor > 0.0:
                quantizer_loss += (
                    max(0.0, 1 - global_step / self.entropy_annealing_steps)
                    * self.entropy_annealing_factor * extra_result_dict["entropy_loss"]
            ...
            total_loss = (
                ...
                + self.perceptual_weight * perceptual_loss
                + self.quantizer_weight * quantizer_loss
                + self.latent_concept_weight * latent_concept_loss
                + d_weight * discriminator_factor * generator_loss
            )
            loss_dict = dict(
                total_loss=total_loss.clone().detach(),
                ...
                perceptual_loss=(self.perceptual_weight * perceptual_loss).detach(),
                quantizer_loss=(self.quantizer_weight * quantizer_loss).detach(),
                latent_concept_loss=(self.latent_concept_weight * latent_concept_loss).detach(),
                weighted_gan_loss=(d_weight * discriminator_factor * generator_loss).detach(),
                discriminator_factor=torch.tensor(discriminator_factor),
                commitment_loss=extra_result_dict["commitment_loss"].detach(),
                codebook_loss=extra_result_dict["codebook_loss"].detach(),
...
class PConceptReconstructionLoss_Single_Stage(ReconstructionLoss_Stage2):
    def __init__(
        self,
        config
    ):
        ...
        loss_config = config.losses
        self.quantize_mode = config.model.vq_model.get("quantize_mode", "vq")
        self.patch_concept_weight = config.losses.get("patch_concept_weight", 0.0)
    ...
    def _forward_generator(self,
        ...
        if self.quantize_mode == "vq":
            # Compute quantizer loss.
            quantizer_loss = extra_result_dict["quantizer_loss"]
            patch_concept_loss =  extra_result_dict["patch_concept_loss"]
            total_loss = (
                reconstruction_loss
                + self.perceptual_weight * perceptual_loss
                + self.quantizer_weight * quantizer_loss
                + self.patch_concept_weight * patch_concept_loss
                + d_weight * discriminator_factor * generator_loss
            )
            loss_dict = dict(
                total_loss=total_loss.clone().detach(),
                ...
                perceptual_loss=(self.perceptual_weight * perceptual_loss).detach(),
                quantizer_loss=(self.quantizer_weight * quantizer_loss).detach(),
                patch_concept_loss=(self.patch_concept_weight * patch_concept_loss).detach(),
                weighted_gan_loss=(d_weight * discriminator_factor * generator_loss).detach(),
                discriminator_factor=torch.tensor(discriminator_factor),
                commitment_loss=extra_result_dict["commitment_loss"].detach(),
                codebook_loss=extra_result_dict["codebook_loss"].detach(),
