...
if hasattr(torch.nn.functional, 'scaled_dot_product_attention'):
    ...
    try:
        ...
        ATTENTION_MODE = 'math'
logger.debug("attention mode is %s", ATTENTION_MODE)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if ATTENTION_MODE == 'flash':
            ...
            raise NotImplementedError(f"Unsupported attention mode {ATTENTION_MODE}")
...
class TiTokEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.cls_token = config.model.vq_model.get("cls_token", True)
        ...
        self.is_latent_concept_loss = config.model.vq_model.get("vit_lcl", False)
        if self.is_latent_concept_loss:
            self.vit_lcl_size = config.model.vq_model.get("vit_lcl_size", 49152)
            self.concept_project = nn.Linear(
                self.width, self.vit_lcl_size, bias=True)

        self.is_latent_cos_loss = config.model.vq_model.get("vit_lcos", False)
        if self.is_latent_cos_loss:
            self.vit_lcos_size = config.model.vq_model.get("vit_lcos_size", 768)
            self.concept_project = nn.Linear(
                self.width, self.vit_lcos_size, bias=True)
    ...
    def forward(self, pixel_values, latent_tokens):
        ...
        x = x.permute(0, 2, 1)
        if self.cls_token:
            x = torch.cat([_expand_token(self.class_embedding, x.shape[0]).to(x.dtype), x], dim=1)  # shape = [*, grid ** 2+1, width]
        ...
        if self.is_latent_concept_loss or self.is_latent_cos_loss:
            latent_tokens_image = latent_tokens.clone()
            concept_proj = self.concept_project(latent_tokens.mean(1))            
        # fake 2D shape
        if self.is_legacy:
        ...
        if self.is_latent_concept_loss or self.is_latent_cos_loss:
            # return latent_tokens,concept_proj, latent_tokens_image #tsne
            return latent_tokens,concept_proj
        else:
...
class TiTokDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.is_patch_concept_loss = config.model.vq_model.get("vit_pcl", False)
        self.patch_concept_layer = config.model.vq_model.get("vit_pcl_layer", 11)

        if self.is_patch_concept_loss:
            self.vit_pcl_size = config.model.vq_model.get("vit_pcl_size", 49152)
            self.concept_project = nn.Linear(
                self.width, self.vit_pcl_size, bias=True)
            self.ln_post_pcl = nn.LayerNorm(self.width)
    ...
    def forward(self, z_quantized):
        ...
        for i in range(self.num_layers):
            x = self.transformer[i](x)
            if self.is_patch_concept_loss and self.patch_concept_layer == i:
                concept_proj = x.permute(1, 0, 2)  # LND -> NLD
                concept_proj = concept_proj[:, :self.grid_size**2]  
                concept_proj = self.ln_post_pcl(concept_proj)
                concept_proj = self.concept_project(concept_proj)
        ...
        # if self.is_patch_concept_loss:
        #     concept_proj = self.concept_project(x)
        ...
        x = self.conv_out(x)
        if self.is_patch_concept_loss:
            return x,concept_proj
        else:
            return x

class TexTTiTokEncoder(TiTokEncoder):
    def __init__(self, config):
    ...
    def forward(self, pixel_values, latent_tokens, text_guidance):
        ...
        latent_tokens = x[:, self.grid_size**2+self.text_context_length:]
        latent_tokens = self.ln_post(latent_tokens)
        if self.is_latent_concept_loss or self.is_latent_cos_loss:
            latent_tokens_image = latent_tokens.clone()
            concept_proj = self.concept_project(latent_tokens.mean(1))
