# Concept Tokenizer note: modified from 1d-tokenizer training scripts (https://github.com/bytedance/1d-tokenizer).
...
# Set PyTorch CUDA memory allocator to use expandable segments to avoid fragmentation
...
from modeling.tatitok import TATiTok, TextEncTiTok
...
def main():
    ...
    if 'tatitok' in config.experiment.tokenizer_checkpoint:
        tokenizer = TATiTok.from_pretrained(config.experiment.tokenizer_checkpoint)
        tokenizer.eval()
        tokenizer.requires_grad_(False)
    else:
        tokenizer = TextEncTiTok(config)
        tokenizer.load_pretrained_weight(config.experiment.tokenizer_checkpoint)
        del tokenizer.latent_concept_loss
        tokenizer.encoder.is_latent_concept_loss = False
        tokenizer.eval()
        tokenizer.requires_grad_(False)
    ...
    if config.model.vq_model.quantize_mode != "vq":
        raise ValueError("This release supports MaskGen_VQ only; set model.vq_model.quantize_mode to 'vq'.")
    ...
    # Prefer dataloader-derived batch count in distributed training to avoid
    # drift between configured max_train_examples and actual sampled batches.
    if hasattr(train_dataloader, "num_batches"):
        num_batches = train_dataloader.num_batches
    else:
        num_batches = len(train_dataloader)
    # Recalculate training steps based on actual dataloader size.
