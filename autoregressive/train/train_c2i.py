# Concept Tokenizer note: modified from GigaTok (https://silentview.github.io/GigaTok/).
# Modified from:
...
from data.dataset import build_dataset, random_crop_arr
...
from utils.lr_schedulers import const_lr, cosine_schedule_with_warmup_v2, wsd_lr
# from tokenizer.tokenizer_image.vq.vq_train import wsd_find_newest_ckpt

from utils.train_utils import (
    get_config, get_titok_tokenizer, create_clip_model,
...
def create_local_tokenizer(args, device):
    ...
    """
    if "titok" in args.save_path:
        tokenizer_model = TiTok(demo_util.get_config(args.tok_config))
    else:
        tokenizer_model = TextEncTiTok(demo_util.get_config(args.tok_config))
 
    tokenizer_model.load_pretrained_weight(args.vq_ckpt)
    if hasattr(tokenizer_model, "latent_concept_loss"):
        del tokenizer_model.latent_concept_loss
        tokenizer_model.encoder.is_latent_concept_loss = False
...
def main(args):
    ...
    if args.dataset == "imagenet":
        ...
        if args.imagenet_caption_cache:
            from io import BytesIO
            import PIL.Image

            cache_dir = args.imagenet_caption_cache
            caption_data_dir = os.path.join(cache_dir, "data")
            train_files = [
                os.path.join(caption_data_dir, f"train-{i:05d}-of-00207.parquet")
                for i in range(207)
            ]
            train_files = [f for f in train_files if os.path.exists(f)]
            if not train_files:
                raise FileNotFoundError(
                    f"No parquet files found under {caption_data_dir} "
                    "(e.g. train-00000-of-00207.parquet)"
                )
            parquet_ds = load_dataset("parquet", data_files={"train": train_files}, split="train")

            class ImagenetCapDatasetWrapper:
                """Loads image, class label, and optional caption from ImageNet caption parquet."""

                def __init__(self, dataset, transform, max_skip=100):
                ...
                def __getitem__(self, idx, _skip_count=0):
                    if _skip_count >= self.max_skip:
                        raise RuntimeError(f"Too many consecutive bad samples (>{self.max_skip}), last idx={idx}")
                    try:
                        row = self.dataset[idx]
                        image = row["image"]
                        label = row["label"]
                        caption = row.get("caption_enriched") or ""
                        ...
                        if isinstance(image, bytes):
                            ...
                            image = PIL.Image.open(BytesIO(image["bytes"])).convert("RGB")
                        elif hasattr(image, "convert"):
                            image = image.convert("RGB")
                        else:
                            raise ValueError(f"Unexpected image type: {type(image)}")
                        return self.transform(image), label, caption
                    except Exception:
                        next_idx = (idx + 1) % len(self.dataset)
                        return self.__getitem__(next_idx, _skip_count + 1)

            dataset = ImagenetCapDatasetWrapper(parquet_ds, transform)
        else:
    ...
    if args.grad_ckpt:
        ...
        model = GPT_models[args.gpt_model](
            vocab_size=tok_config["model"]["vq_model"]["codebook_size"],
            block_size=tok_config["model"]["vq_model"]["num_latent_tokens"],
            num_classes=args.num_classes,
            cls_token_num=args.cls_token_num,
            model_type=args.gpt_type,
            resid_dropout_p=dropout_p,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch in loader:
            x, y = batch[0], batch[1]
            text_list = list(batch[2]) if len(batch) == 3 else [f"A photo of a {imagenet_idx2classname[c.item()]}." for c in y]
            x = x.to(device, non_blocking=True)
            if args.dataset != 'imagenet_code':
                with torch.no_grad():
                    if args.quant_way == 'vq':
                        if "titok" in args.save_path:
                            indices = tokenizer.encode(x)[1]["min_encoding_indices"]
                        else:
                            text = text_list
                            ...
                            indices = tokenizer.encode(x, text_guidance)[1]["min_encoding_indices"]
                    else:
                        raise NotImplementedError(f"Unsupported quantization mode {args.quant_way}")
                    # elif args.quant_way == 'fsq':
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional number of pre-tokenized code examples. Defaults to scanning code-path for matching .npy files.",
    )
    parser.add_argument("--data-path", type=str, default=None)
    parser.add_argument("--imagenet-caption-cache", type=str, default=os.environ.get("IMAGENET_CAPTION_CACHE"), help="optional ImageNet caption parquet cache root")
