...
def quantitative_eval(args, independent=False):
    ...
    num_token = tok_config["model"]["vq_model"]["num_latent_tokens"] if args.num_token is None else args.num_token
    codebook_size = tok_config["model"]["vq_model"]["codebook_size"]
    codebook_embed_dim = tok_config["model"]["vq_model"]["token_size"]
    is_patch_concept_loss = tok_config["model"]["vq_model"]["vit_pcl"] if "vit_pcl" in tok_config["model"]["vq_model"] else False
    ...
    # Include annotation in folder name to avoid conflicts between different annotations
    annotation = ";".join([f"{key}={value:.2f}" for key, value in args.cfg_schedule_kwargs.items()])
    annotation_safe = annotation.replace(";", "_").replace("=", "_").replace(".", "_")
    folder_name = f"{model_string_name}-size-{args.image_size}-size-{args.image_size_eval}-" \
                  f"topk-{args.top_k}-topp-{args.top_p}-temperature-{args.temperature}-" \
                  f"cfg-{args.cfg_scale}-{args.cfg_schedule}-{annotation_safe}-seed-{args.global_seed}"
    ...
    for _ in pbar:
        ...
        if args.quant_way == 'vq':  
            if is_patch_concept_loss:
                samples = tokenizer_model.decode_tokens(index_sample)[0]
            else:
                samples = tokenizer_model.decode_tokens(index_sample) #, qzshape) # output value is between [-1, 1]
            t2 = time.time()
...
def search(args):
    ...
    if args.cfg_schedule == 'rectangular':
        ...
        window_end_init = 0.6  # You can adjust this initial value
        ...
        for window_start, window_end in tqdm(combs, total=len(combs), desc="seaching progress"):
            print(f"searching for window_start={window_start}, window_end={window_end}")
            cfg_schedule_kwargs = {
...
def get_next_search_state_cfg(
    ...
    try:
        ...
        print(f"The file {csv_file_path} is empty.")
        return initial_cfg_scale, None,  False
    print(f"cfg_schedule: {cfg_schedule}, annotation: {annotation}")
    # Filter the DataFrame based on cfg_schedule
    df_filtered = df[df["cfg_schedule"] == cfg_schedule]
    # change the nan to ""
    df_filtered = df_filtered.fillna("")
    ...
    for scale in potential_next_scales:
        if scale not in tried_scales:
            print(f"Trying scale {scale} which has not been tried yet.")
    ...
    print(f"All neighboring scales have been tried, implying a local minimum.")
