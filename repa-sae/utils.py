...
class SAE(nn.Module):
    ...
    def __init__(self, dirs):
        ...
        sae_checkpoint = torch.load(dirs,  map_location="cpu")
        self.sae_W_enc = nn.Parameter(sae_checkpoint['W_enc'])
        self.sae_b_enc = nn.Parameter(sae_checkpoint['b_enc'])
        self.sae_b_dec = nn.Parameter(sae_checkpoint['b_dec'])
        for param in self.parameters():
            param.requires_grad = False
 
    def forward(self, patch_tokens):
        sae_in = patch_tokens - self.sae_b_dec[None,None,:] # [B, T, D]
        sae_hidden = sae_in @ self.sae_W_enc+self.sae_b_enc[None,None,:] # [B, T, D]
        return sae_hidden
