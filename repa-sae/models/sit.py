...
def build_sae_projector(hidden_size, projector_dim, z_dim):
...
class SiTBlock(nn.Module):
    ...
    def _forward_attn_with_matrix(self, x):
        B, N, C = x.shape
    ...
    def forward(self, x, c, return_attn=False):
        ...
        if return_attn:
            attn_out, attn_matrix = self._forward_attn_with_matrix(attn_input)
        else:
...
class SiT(nn.Module):
        ...
        if self.use_projector:
            if use_sae:
                self.projectors = nn.ModuleList([
                    build_sae_projector(hidden_size, projector_dim, z_dim) for z_dim in z_dims
                    ])
            else:    
                self.projectors = nn.ModuleList([
    ...
    def forward(self, x, t, y, return_logvar=False, return_encoder_attn=False):
        """
        Forward pass of SiT.
        ...
        for i, block in enumerate(self.blocks):
            if return_encoder_attn and (i + 1) == self.attn_layer:
                x, encoder_attn = block(x, c, return_attn=True)   # (N, T, D), (N, H, T, T)
            else:
                x = block(x, c)                      # (N, T, D)
            if (i + 1) == self.encoder_depth and not return_encoder_attn and self.use_projector:
                zs = [projector(x.reshape(-1, D)).reshape(N, T, -1) for projector in self.projectors]
        x = self.final_layer(x, c)                # (N, T, patch_size ** 2 * out_channels)
        x = self.unpatchify(x)                   # (N, out_channels, H, W)
        if return_encoder_attn:
